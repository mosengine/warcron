...
@require_once ('../../inc/languages/'.$_COOKIE['SITE_LANG'].'/conf.main.php');
...
<body>
	...
	$ulvlh=verifylevel($_REQUEST['UID']);
	$qquery = mysql_query("SELECT id_forum FROM `forums` WHERE viewlevel<=".$ulvlh." GROUP BY `group`") OR DIE(mysql_error());
	echo mysql_num_rows($qquery);
	...
	var TargetLoc="menuForums";				// span id for relative positioning
...
<?php
	...
	$qquery = mysql_query("SELECT `group` FROM `forums` WHERE viewlevel<=".$ulvlh." GROUP BY `group` ORDER BY `group` ASC") OR DIE(mysql_error());
	$i=1;
	while ($rowx = mysql_fetch_array($qquery)) {
		$j=1;
			$qqueryb = mysql_query("SELECT `id_forum`, title FROM `forums` WHERE viewlevel<=".$ulvlh." AND `group`='".$rowx['group']."' ORDER BY `ordenation` ASC") OR DIE(mysql_error());
			echo '
			Menu'.$i.'=new Array(bulletImg+"'.$FORUM_GROUP[$rowx['group']].'"+underLine,"?n=forums",menuBg,'.mysql_num_rows($qqueryb).',15,140,"transparent","","","","transparent","",-1,-1,-1,"","");';
			while ($rowc = mysql_fetch_array($qqueryb)) {
				echo '
				Menu'.$i.'_'.$j.'=new Array(bulletImg+"'.$rowc['title'].'"+underLine,"?n=forums&f='.$rowc['id_forum'].'",menuBg,0,15,150,"","","","","","",-1,-1,-1,"","");';
				$j++;
			}
		$i++;
	}
