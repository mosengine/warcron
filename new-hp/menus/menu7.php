...
$query = "SELECT DATE_FORMAT(`date`, '%Y-%m') as `date` FROM forum_posts fp
		...
		WHERE DATEDIFF(NOW(), fp.`date`) < 365 AND fp.isreply=0 AND ft.category=2 AND viewlevel <='".verifylevel($_SESSION['userid'])."'
		GROUP BY fp.`id_post` 
		ORDER BY `date` ASC";
...
$strdate = explode (',', $strdate);
 ?>
Menu1=new Array(bulletImg+"Spotlight"+underLine,"?n=community.spotlight",menuBg,<?php echo count($strdate)-1; ?>,15,140,"transparent","","","","transparent","",-1,-1,-1,"","");
<?php
	for ($i=0; $i<count($strdate)-1;$i++) {
		...
		echo 'Menu1_'.($i+1).'=new Array(bulletImg+"'.$ddate.'"+underLine,"?n=community.spotlight&m='.$ddate2.'",menuBg,0,15,125,"","","","","","",-1,-1,-1,"","");';
	}
	$query = mysql_query("SELECT id FROM web_online");
?>
