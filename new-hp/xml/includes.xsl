...
<xsl:include href="user-menu.xsl" />
...
<head>
	...
	<style type="text/css" media="screen, projection">
		@import "/new-hp/css/armory/master.css";
		@import "new-hp/css/armory/<xsl:value-of select="$lang"/>/language.css";
		@import "/shared/global/menu/topnav/topnav.css";
		<xsl:if test="not(/page/pageIndex)">
			@import "/new-hp/css/armory/int.css";
		</xsl:if>
	...
	<script type="text/javascript" src="/new-hp/js/common.js"></script>
	<script type="text/javascript" src="/new-hp/js/tooltip_compact.js"></script>
	...
	<script type="text/javascript">
		//browser detection		
		if($.browser.msie){
			if($.browser.version == "7.0")		addStylesheet('/new-hp/css/browser/ie7.css');
			if($.browser.version == "6.0")		addStylesheet('/new-hp/css/browser/ie.css');
		}else if($.browser.mozilla){
			if(parseFloat($.browser.version) &lt;= 1.9)	addStylesheet('/new-hp/css/browser/firefox2.css');
		}else if($.browser.opera)				addStylesheet('/new-hp/css/browser/opera.css');
		else if($.browser.safari)				addStylesheet('/new-hp/css/browser/safari.css');
		...
		var isHomepage = ("<xsl:value-of select="not(/page/pageIndex)" />" != "false") ? false : true ;
...
<body>
	...
	<script type="text/javascript" src="/new-hp/js/dhtmlHistory.js"></script>
	<script type="text/javascript" src="/new-hp/js/_lang/{$lang}/strings.js"></script>
...
<xsl:template name="searchInput">
	<div class="search-module">
		...
		<form name="formSearch" action="/armory/search.xml" method="get" onSubmit="javascript: return menuCheckLength(document.formSearch);">
			<input id="armorySearch" type="text" name="searchQuery" value="" size="16" maxlength="72" />
			<a href="javascript:void(0);" class="submit" onclick="javascript: return menuCheckLength(document.formSearch);">
...
<xsl:template name="footer">
	...
	<div id="languageFooter" class="language">
		<div class="module">
			...
			<xsl:variable name="languageLinks" select="document('/new-hp/xml/data/languages.xml')" />
...
<xsl:choose>
	<xsl:when test="/page/armorySearch/searchResults/arenaTeams">
		<xsl:call-template name="related-info">
			<xsl:with-param name="src" select="concat('/new-hp/xml/strings/',$lang,'/ri-arena-ladder.xml')"/>
		</xsl:call-template>
	</xsl:when>
	<xsl:when test="/page/armorySearch/searchResults/characters">
		<xsl:call-template name="related-info">
			<xsl:with-param name="src" select="concat('/new-hp/xml/strings/',$lang,'/ri-character-sheet.xml')"/>
		</xsl:call-template>
	</xsl:when>
	<xsl:when test="/page/armorySearch/searchResults/guilds">
		<xsl:call-template name="related-info">
			<xsl:with-param name="src" select="concat('new-hp/xml/strings/',$lang,'/ri-guild-info.xml')"/>
		</xsl:call-template>
	</xsl:when>
	<xsl:when test="$isRecommended = 1">
		<xsl:call-template name="related-info">
			<xsl:with-param name="src" select="concat('/new-hp/xml/strings/',$lang,'/ri-item-info-recommended.xml')"/>
		</xsl:call-template>
	</xsl:when>
	<xsl:when test="/page/armorySearch/searchResults/items">
		<xsl:call-template name="related-info">
			<xsl:with-param name="src" select="concat('/new-hp/strings/',$lang,'/ri-item-info.xml')"/>
		</xsl:call-template>
	</xsl:when>
	<xsl:when test="/page/characterInfo/characterTab">
		<xsl:call-template name="related-info">
			<xsl:with-param name="src">
				<xsl:choose>
					<xsl:when test = "$isPropass != 'true'">/new-hp/strings/<xsl:value-of select = "$lang" />/ri-character-sheet.xml</xsl:when>
	...
	<xsl:when test="/page/characterInfo/reputationTab">
		<xsl:call-template name="related-info">
			<xsl:with-param name="src" select="concat('/new-hp/xml/strings/',$lang,'/ri-character-reputation.xml')"/>
		</xsl:call-template>
	</xsl:when>
	<xsl:when test="/page/statistics or /page/achievements">
		<xsl:call-template name="related-info">
			<xsl:with-param name="src" select="concat('/new-hp/xml/strings/',$lang,'/ri-achievements.xml')"/>
		</xsl:call-template>
	</xsl:when>
	<xsl:when test="/page/characterInfo/talentGroups">
		<xsl:call-template name="related-info">
			<xsl:with-param name="src">
				<xsl:choose>
	...
	<xsl:when test="/page/teamInfo/arenaTeam">
		<xsl:call-template name="related-info">
			<xsl:with-param name="src" select="concat('/new-hp/xml/strings/',$lang,'/ri-team-info.xml')"/>
		</xsl:call-template>
	</xsl:when>
	<xsl:when test="/page/selectTeamType">
		<xsl:call-template name="related-info">
			<xsl:with-param name="src" select="concat('/new-hp/xml/strings/',$lang,'/ri-arena-calculator.xml')"/>
		</xsl:call-template>
	</xsl:when>
	...
	<xsl:when test="/page/@requestUrl='/vault/character-calendar.xml'">
		<xsl:call-template name="related-info">
			<xsl:with-param name="src" select="concat('/new-hp/xml/strings/',$lang,'/ri-calendar.xml')" />
		</xsl:call-template>
	</xsl:when>
	<xsl:when test="/page/@requestUrl='/arena-team-game-chart.xml'">
		<xsl:call-template name="related-info">
			<xsl:with-param name="src" select="concat('/new-hp/xml/strings/',$lang,'/ri-arena-reports.xml')" />
		</xsl:call-template>
	</xsl:when>
	<xsl:when test="/page/@requestUrl='/arena-team-report-opposing-teams.xml'">
		<xsl:call-template name="related-info">
			<xsl:with-param name="src" select="concat('/new-hp/xml/strings/',$lang,'/ri-opponent-history.xml')" />
		</xsl:call-template>
	</xsl:when>
	<xsl:when test="/page/@requestUrl='/arena-game.xml'">
		<xsl:call-template name="related-info">
			<xsl:with-param name="src" select="concat('/new-hp/xml/strings/',$lang,'/ri-match-report.xml')" />
		</xsl:call-template>
	</xsl:when>
	<xsl:otherwise>
		<xsl:call-template name="related-info">
			<xsl:with-param name="src" select="concat('/new-hp/xml/strings/',$lang,'/ri-armory.xml')"/>
		</xsl:call-template>
	</xsl:otherwise>
</xsl:choose>
</xsl:template>
