...
function hidePin() {
    ...
    if (!window.theCharUrl) {
		...
		document.getElementById('replacePinGuildName2').innerHTML = "<a href = 'index.php?searchType=registration' onMouseOver = \"javascript: showTip(textLearnMoreHover);\" onMouseOut = 'hideTip();'>&lt; "+ textLearnMore +" &gt;</a>";
	} else {
		setPinThisChar();
	}
...
function tipPosition(callingEvent) {
  ...
  mouseY = callingEvent.clientY-1;
	...
	if (isShowing) {
		...
		if(ttipSideOvershoot > windowWidth) {
				if ((is_opera) && (elemt1c.offsetWidth > 299)) {
						elemttc.style.left = mouseX-elemttc.offsetWidth-30+"px";
				} else {
						elemttc.style.left = windowWidth - elemttc.offsetWidth+"px";
				}
		} else {
...
function menuCheckLength(formReference) {
	...
	if(IS_ENABLED_XSLT){
		...
		window.location.href = "index.php?searchQuery="+theSearch+"&searchType="+formReference.searchType.value;
