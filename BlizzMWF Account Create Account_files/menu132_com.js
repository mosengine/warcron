...
if(document.getElementById){
	...
	if((AgntUsr.indexOf("opera")==-1||Opr7)){
		...
		InitLdd,MstSz=0;
		if(parent.operaBool==1) //  OperaBool addresses Opera issue where menu is loaded twice.
...
function ChckInitLd(){
	...
	if(InitLdd && (parent.menuCookie[menuNumber-1]==1 || !MenuUsesFrames))	{clearInterval(LdTmr);Go()}} //Controls selective initial loading;
...
function GoTo(){
	if(UnfoldsOnClick&&!M_Clckd&&this.Arr[3]){M_Clckd=true;OpenMenuII(this)}
		...
		LowItem(this);IniFlg=true;Initiate();
...
try{parent.menuClear(menuNumber);}catch(err){}
	if(Ldd&&Crtd){
		...
		CurOvr=this;IniFlg=false;ClrAllChlds(this.Ctnr.FrstMbr);HiliteItem(this);onBool=true;
